...
pub enum ForwardMatch {
    ...
    Extrapolated {
        nearest_listed_expiry_years: f64,
    },
}
    ...
    pub fn forward_for(&self, expiry_years: f64) -> ForwardLookup {
        ...
        if expiry_years <= first {
            ...
            return if (expiry_years - first).abs() < 1e-9 {
                ForwardLookup {
                    forward,
                    match_kind: ForwardMatch::Exact,
                }
            } else {
                ForwardLookup {
                    forward,
                    match_kind: ForwardMatch::Extrapolated {
                        nearest_listed_expiry_years: first,
                    },
                }
            };
        }
        if expiry_years >= last {
            ...
            return if (expiry_years - last).abs() < 1e-9 {
                ...
                ForwardLookup {
                    forward,
                    match_kind: ForwardMatch::Extrapolated {
                        nearest_listed_expiry_years: last,
                    },
                }
            };
        }
        ...
        ForwardLookup {
            forward,
            match_kind: ForwardMatch::Interpolated,
        }
    }
}
...
impl BookRisk {
    ...
    pub fn local_gamma_near(
        &self,
        target_log_moneyness: f64,
        expiry_years: f64,
        bandwidth: f64,
    ) -> f64 {
...
pub fn aggregate(
    ...
    for pos in positions {
        ...
        *risk.by_expiry.entry(bucket_key(t)).or_default() += g;
        risk.by_strike.push(StrikeGamma {
            log_moneyness,
            expiry_years: t,
            gamma: g.gamma,
        });
...
mod tests {
    ...
    fn local_gamma_weighs_a_nearby_strike_more_than_a_distant_one() {
        ...
        let near_strike = OptionKey {
            option_type: OptionType::Call,
            strike: 65000.0,
            expiry_years: expiry,
        }; // ATM
        let far_strike = OptionKey {
            option_type: OptionType::Call,
            strike: 90000.0,
            expiry_years: expiry,
        }; // way OTM
        let positions = vec![
            OptionPosition {
                instrument: near_strike,
                size: 10.0,
            },
            OptionPosition {
                instrument: far_strike,
                size: 10.0,
            },
        ];
        ...
        let near_only_gamma = risk
            .by_strike
            .iter()
            .find(|p| (p.log_moneyness).abs() < 1e-9)
            .unwrap()
            .gamma;
        ...
        assert!(
            (local - near_only_gamma).abs() < near_only_gamma.abs() * 0.05,
            "local={local} near_only={near_only_gamma}"
        );
    }
    ...
    fn local_gamma_ignores_positions_in_a_different_expiry_bucket() {
        ...
        let short_dated = OptionKey {
            option_type: OptionType::Call,
            strike: 65000.0,
            expiry_years: 7.0 / 365.0,
        };
        let long_dated = OptionKey {
            option_type: OptionType::Call,
            strike: 65000.0,
            expiry_years: 60.0 / 365.0,
        };
        let positions = vec![
            OptionPosition {
                instrument: short_dated,
                size: 10.0,
            },
            OptionPosition {
                instrument: long_dated,
                size: 10.0,
            },
        ];
        let risk = aggregate(&positions, &surface, &forwards);
        ...
        assert!(
            (local_long - long_only_gamma).abs() < 1e-9,
            "local={local_long} long_only={long_only_gamma}"
        );
    }
    ...
    fn forward_curve_interpolates_between_listed_expiries() {
        ...
        assert!(
            (lookup.forward - 67500.0).abs() < 1e-6,
            "forward={}",
            lookup.forward
        );
    ...
    fn forward_curve_flags_extrapolation_instead_of_hiding_it() {
        ...
        assert_eq!(
            lookup.match_kind,
            ForwardMatch::Extrapolated {
                nearest_listed_expiry_years: 60.0 / 365.0
            }
        );
    }
    ...
    fn single_listed_expiry_extrapolates_flat_in_both_directions() {
        ...
        assert_eq!(
            forwards.forward_for(14.0 / 365.0).match_kind,
            ForwardMatch::Exact
        );
    }
}
